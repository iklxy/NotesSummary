...
def generate_ca_cells_for_question(
    ...
    user_prompt = (
        ...
        "   - 格式优先为：\"- 40%\"、\"- 40%（其中非共价 0%）\"、\"- 约 30%-40%\"、\"- 5 例\"。\n"
        ...
        "   - 如果无法确认明确数字，answer 写 \"/\"，numeric_value 设为 null。\n"
        "   - 示例：\n"
        "     问题：初治CLL BTKi单药治疗占比\n"
...
def generate_ca_row_summary_for_question(
    ...
    "7. 每类观点后需要补充一点关键细节，说明该观点具体体现在哪里，但不要展开成长段分析。\n"
    "8. 如果存在少数不同观点，也要单独输出一条，例如“1/5:认为xxx”。\n"
    "9. 不要机械罗列每个访谈编号，不要写成“访谈1认为、访谈2认为……”。\n"
    "10. 多类观点之间用中文分号“；”分隔，不要使用 markdown 列表。\n"
    "11. 定性问题总结优先采用：\"4/5:支持A观点，认为xxx；1/5:认为B观点，认为xxx。\"\n"
    "12. 定量问题总结可按数值区间或结论倾向聚类，例如：\"3/5:集中在30%-40%，认为该比例处于较高水平；2/5:低于20%，认为实际使用仍有限。\"；不要重复统计列中已经计算好的复杂统计结果。\n"
    "13. 总结控制在 1 到 2 句，尽量短而直接。\n"
    "14. 不要输出引用、不要输出列表、不要输出 markdown。\n"
    "15. 只输出 JSON，结构必须是 {\"summary\": \"...\"}。\n"
    "16. 输出示例：\n"
    "    {\"summary\": \"4/5:支持A观点，认为疗效和安全性共同影响治疗选择；1/5:认为B观点，认为患者支付能力和药物可及性影响更大。\"}\n"
)
    raw_output = generate_fn(system_prompt, user_prompt)
