...
def generate_ca_cells_for_question(
    ...
    user_prompt = (
        ...
        "     正确输出：\"- 分期和可手术性\\n- PD-L1检测结果\\n- ctDNA检测结果\"\n"
        "5. 证据引用输出若干条都可以，如果只有 1 条直接相关原文，也必须正常作答，不得因引用不足输出 /。\n"
        ...
        "### 【核心信息判定强制规则（必须严格执行）】\n"
        "10.只要原文中有明确相关信息，就应尽量输出完整/部分答案，严禁因问题措辞、关键词与原文不完全一致就返回 /。"
        "11.只有在全文 Notes 中完全找不到任何可支持该问题的信息时，才允许输出 /。\n"
        "12.信息跨句子、跨段落、表述语序不同时，正常提取整合作答，不得以此为由输出 /。\n"
        "13.原文使用同义、近义表述，未使用问题中原样关键词时，仍判定为有效信息，不得判定为未提及并输出 /。\n"
        "14.若原文信息不完整、无法回答全部问题，遵循「能回答多少就回答多少」原则，不得因信息不全直接输出 /。\n"
        "15.只要原文存在能直接支撑该问题的相关片段，就必须正常作答；仅通篇无有效支撑内容时，方可使用 /。\n"
        "### 【执行红线（严禁违规）】\n"
        "- 禁止主观判断“关联性弱”而刻意填写 /。\n"
        "### 【前置强制校验步骤（必做）】\n"
        "Step1 通读完整访谈notes，逐句筛查是否存在和问题**语义相关**的内容，不局限于字面关键词；\n"
        "Step2 只要筛查到任意一处相关内容，立刻提取信息作答，**绝对禁止输出 /**；\n"
        "Step3 仅当通篇读完，无任何一句相关内容，才可输出 /。\n"
        "### 【反面示例（明确错误行为）】\n"
        "例1：问题关键词为“用药考量”，原文写“选择药物的参考条件”，语义一致 → 必须作答，禁止输出 /\n"
        "例2：相关内容拆分在3个段落、5个句子中 → 整合提炼作答，禁止输出 /\n"
        "例3：原文只能回答问题一半内容 → 输出已有内容，禁止输出 /\n"
        "例4：主观觉得内容关联度不高，但语义确实匹配问题 → 正常作答，禁止输出 /\n"
        "### 【最终兜底要求】\n"
        "默认优先判定「有相关内容」，宁可提取部分信息作答，也绝不轻易输出 /。\n"
