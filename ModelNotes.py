...
def generate_ca_notes_framework(
    ...
    user_prompt = (
        ...
        "2. 框架维度尽量完全包含Notes中的一级标题、二级标题、重要高频子问题，不遗漏、不重复。\n"
        "3. 每个主题分组下的行要尽量细，一行只对应一个可独立回答的问题点。\n"
        "4. 定性问题用可快速扫读的陈述式标题，避免冗长问句。\n"
        "5. 定量问题要尽量拆成单一指标的行，避免把多个指标混在同一行。\n"
        "6. 主题分组数量控制在 5 到 12 个，每组 2 到 5 行。\n"
        "7. 只输出 JSON，不要输出解释、不要输出 markdown。\n"
        "8. 所有 summary 字段必须是自然语言正文，不要输出 JSON 对象或字段名。\n"
        "9. 每一行都要显式标注 question_type，取值只能是 qualitative 或 quantitative。\n"
        "JSON 结构参考如下：\n"
        "{\n"
        '  "groups": [\n'
        "    {\n"
