...
def _build_dg_minutes_outline_prompts(
    ...
    system_prompt = (
        ...
        "-不要输出对问卷的甄别过程内容，直接输出访谈纪要内容\n"
        "-不要输出任何你不完全确定的排序关系，包括比大小、先后、优先级、排序等。\n"
        "-原文中有提及但是并没有完全提及的内容也不要参与排序\n"
        "-只对原文中有明确排序关系的内容进行排序\n"
        "-在全文纪要的最后输出 ”全文纪要结束（BenHealth），并且输出全文的字数\n\n"
        "请再次注意以下要求：\n"
        "1. 原文复刻原则：所有内容需100%来源于访谈原文，如实呈现原文明确提及的表述、数值、观点，不增删、不修改、不延伸语义。\n"
        "2. 禁止自主计算：严禁AI通过原文已有数值进行任何推导、计算、换算（例：原文仅提及“某类人群误诊率不超过30%-40%”，仅保留该原文表述，不得计算、补充“正确率为60%-70%”；原文未提及相关数值计算结果，不得自行生成）。\n"
        "3. 禁止主观推导：严禁AI自行推导、补充原文未明确提及的信息（例：原文未提及“换药方向”相关具体比例，不得新增“90%以上的换药为升阶换药”等任何推导性表述，仅保留原文已有相关内容）。\n"
        "4. 数值与表述规范：原文中的关键数据、专家原话、专业术语需完整复刻，不得擅自修改、替换，不得添加任何AI自主判断的表述。\n"
        "5. 杜绝任何形式的\n"
        "6. 严厉禁止AI自主加工、计算、推导，一定要确保纪要的真实性、客观性和准确性。\n"
        "7. 在访谈纪中，不需要输出“甄别问卷”这一部分，直接顺序输出访谈纪要内容即可。\n"
        "8. 在全文纪要的最后输出 ”全文纪要结束（BenHealth），并且输出全文的字数\n\n"
    )
    user_prompt = (
        ...
        "3. 不要输出对问卷的甄别过程内容，直接输出访谈纪要内容\n"
        "4. 在全文纪要的最后输出 ”全文纪要结束（BenHealth），并且输出全文的字数\n\n"
    )
    return system_prompt, user_prompt
    ...
    if not payload.get("sections"):
        ...
        if questionnaire_text_clean:
            retry_user_prompt = (
                ...
                "6. 在智能纪要中不要输出你不确定或者不明晰的排序关系，包括比大小、排序等。\n"
                "7. DG 中未覆盖但访谈中出现的新主题可以保留，但仍需按语义顺序插入并标注为 DG 之外的内容。\n\n"
                "【讨论指南（DG）/ 访谈问卷】\n"
                "以下内容是本次访谈使用的讨论指南：\n\n"
...
def generate_kbq_dimensions(
    ...
    if normalized_user_dimensions:
        system_prompt = (
            ...
            "你的任务是基于访谈原文以及用户已定义的分析维度，一共生成2 到 4个分析维度，新生成的维度不允许和用户已定义的维度重复。"
            "只输出严格合法的 JSON，不要输出额外说明。"
        )
        user_prompt = (
            f"{project_context_block}"
            f"{interview_context_block}"
            "下面是一条 key BQ。用户已经提供了必须覆盖的分析维度（子维度或子问题）。"
            "请在不重复、不改写用户维度的前提下，补充 2 到 4 个额外维度。\n\n"
            f"【key BQ】\n{key_bq_text}\n\n"
            f"【用户已定义的维度】\n{user_dimensions_block}\n\n"
            "规则：\n"
            "1. 维度必须 仅来自原文实际讨论内容，不编造、不新增\n"
            "2. 原文未提及的维度，不生成、不展示，也不标注未提及。\n"
            "3. 维度数量按原文实际情况灵活确定，无需固定数量。\n"
            "4. 如果用户已定义的维度已经足够，可以返回空数组。\n"
            "5. 维度应当是可操作的分析框架，而不是空泛标签。\n"
            "6. 不要输出不必要的解释。\n"
            "维度要求：\n"
...
def generate_ca_notes_framework(
    ...
    "框架必须充分覆盖 Notes 中出现的所有重要主题、子主题、决策因素、评价维度、流程节点和定量指标。"
    "必须输出严格合法的 JSON，不要输出额外说明。"
)
    ...
    "2. 框架维度必须尽量完整覆盖 Notes 中的一级标题、二级标题、重要高频子问题、关键差异点、决策因素、评价维度、治疗流程节点和定量指标，不遗漏重要内容。\n"
    "3. 每个主题分组下的行必须细粒度拆分，一行只对应一个可独立回答的问题点，不要把多个问题点合并成一行。\n"
    "4. 如果一个主题下同时包含适应症、患者类型、治疗选择、使用比例、优劣势、影响因素、未来预期等内容，必须拆成多行，而不是合并成一个宽泛标题。\n"
    "5. 定性问题用可快速扫读的陈述式标题，避免冗长问句。\n"
    "6. 定量问题必须拆成单一指标行，一个行标题只对应一个数字或比例，例如占比、人数、频率、评分、增长/下降幅度等。\n"
    "7. 行标题不要过度概括，避免使用“整体情况”“综合评价”“治疗现状”“主要观点”等泛化标题。\n"
    "8. 主题分组数量建议控制在 6 到 15 个；每组建议 3 到 8 行。覆盖完整性优先于数量限制，如果 Notes 内容较多，可以适当超过。\n"
    "9. 每个主题分组下至少输出 3 行，除非该主题在 Notes 中信息确实很少。\n"
    "10. 如果某个主题只能拆出 1 到 2 行，优先考虑与相近主题合并；如果某个主题信息丰富，必须继续细拆到 5 行以上。\n"
    "11. 输出前自检：是否存在一个行标题包含多个并列问题点；如果存在，必须继续拆分。\n"
    "12. 输出前自检：是否遗漏 Notes 中反复出现或具有差异价值的内容；如果遗漏，必须补充为新的分析行。\n"
    "13. 只输出 JSON，不要输出解释、不要输出 markdown。\n"
    "14. 所有 summary 字段必须是自然语言正文，不要输出 JSON 对象或字段名。\n"
    "15. 每一行都要显式标注 question_type，取值只能是 qualitative 或 quantitative。\n"
    "16. JSON 结构参考如下：\n"
    "{\n"
    '  \"groups\": [\n'
    "    {\n"
